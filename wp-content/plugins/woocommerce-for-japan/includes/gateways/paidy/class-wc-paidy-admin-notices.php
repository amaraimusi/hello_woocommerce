...
class WC_Paidy_Admin_Notices {
	...
	public function paidy_check_environment() {
		...
		$testmode           = ( isset( $options['environment'] ) && 'sandbox' === $options['environment'] ) ? true : false;
		...
		if ( isset( $options['enabled'] ) && 'yes' === $options['enabled'] ) {
			if ( empty( $show_curl_notice ) ) {
				if ( ! function_exists( 'curl_init' ) ) {
					$this->add_admin_notice( 'curl', 'notice notice-warning', __( 'WooCommerce Paidy - cURL is not installed.', 'woocommerce-for-japan' ), true );
				}
			}
            if ( $testmode ) {
                if (
