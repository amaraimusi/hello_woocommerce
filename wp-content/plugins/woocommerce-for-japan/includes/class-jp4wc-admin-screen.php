...
class JP4WC_Admin_Screen {
	...
	function jp4wc_setting_init(){
        ...
        add_settings_field(
            'jp4wc_options_ppec',
            __( 'PayPal Checkout', 'woocommerce-for-japan' ),
            array( $this, 'jp4wc_options_ppec' ),
            'jp4wc_payment',
            'jp4wc_payments'
        );
	...
	public function jp4wc_options_yomigana() {
		...
		$descritpion = $this->jp4wc_description_address_pattern( $title );
		$this->jp4wc_plugin->jp4wc_input_checkbox('yomigana', $descritpion, $this->prefix);
	}
	/**
	...
	public function jp4wc_options_yahoo_app_id() {
		...
		$description = sprintf(__( 'If you use it a bit for testing, you do not need to enter it here. But if you want to use Automatic zip code entry, you must get and input %s here. Please get it from <a href="https://e.developer.yahoo.co.jp/dashboard/" target="_blank">here</a>. If you need our support, please access <a href="https://wc.artws.info/shop/setting-support/yahoo-app-id/" target="_blank">here</a> and pay for an aid to development costs ', 'woocommerce-for-japan' ), $title);
     ...
     * @return mixed
     */
    public function jp4wc_options_ppec() {
        $title = __( 'PayPal Checkout', 'woocommerce-for-japan' );
        $description = $this->jp4wc_plugin->jp4wc_description_payment_pattern( $title );
        $this->jp4wc_plugin->jp4wc_input_checkbox('ppec', $description, $this->prefix);
    }
...
//			return $input;
	...
	public function admin_enqueue_scripts( $page ) {
		wp_register_style( 'custom_jp4wc_admin_css', JP4WC_URL_PATH . 'includes/views/css/admin-jp4wc.css', false, JP4WC_VERSION );
		wp_enqueue_style( 'custom_jp4wc_admin_css' );
		if ( $page === 'woocommerce_page_wc4jp-options' ) {
			wp_enqueue_script( 'jp4wc-admin-script', JP4WC_URL_PATH. 'includes/views/js/admin-settings.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable', 'jquery-ui-slider', 'jquery-ui-button' ), JP4WC_VERSION );
