...
abstract class WC_Gateway_PPEC extends WC_Payment_Gateway {
		...
		if ( false !== $cert ) {
			...
			if ( false !== $certinfo ) {
				...
				if ( $valid_until < time() ) {
					...
					$expires = '<span style="color: red;">' . __( 'expired on %s', 'woocommerce-for-japan' ) . '</span>';
				} elseif ( $valid_until < ( time() - 2592000 ) ) {
					// Also display in red if the cert is going to expire in the next 30 days
					$expires = '<span style="color: red;">' . __( 'expires on %s', 'woocommerce-for-japan' ) . '</span>';
				} else {
					// Otherwise just display a normal message
					$expires = __( 'expires on %s', 'woocommerce-for-japan' );
				}

				$expires = sprintf( $expires, date_i18n( get_option( 'date_format' ), $valid_until ) );
				$out = sprintf( __( 'Certificate belongs to API username %1$s; %2$s', 'woocommerce-for-japan' ), $certinfo['subject']['CN'], $expires );
			} else {
				$out = __( 'The certificate on file is not valid.', 'woocommerce-for-japan' );
			}
		}
	...
	protected function validate_active_credentials() {
		...
		if ( ! empty( $username ) ) {
			...
			if ( empty( $password ) ) {
				WC_Admin_Settings::add_error( __( 'Error: You must enter API password.', 'woocommerce-for-japan' ) );
				return false;
			}
			...
			if ( is_a( $creds, 'WC_Gateway_PPEC_Client_Credential_Signature' ) && $creds->get_signature() ) {
				...
				try {
					...
					if ( ! $payer_id ) {
						WC_Admin_Settings::add_error( __( 'Error: The API credentials you provided are not valid.  Please double-check that you entered them correctly and try again.', 'woocommerce-for-japan' ) );
						return false;
					}
				} catch ( PayPal_API_Exception $ex ) {

					WC_Admin_Settings::add_error( __( 'An error occurred while trying to validate your API credentials.  Unable to verify that your API credentials are correct.', 'woocommerce-for-japan' ) );
				}
				...
				if ( false === $cert ) {
					WC_Admin_Settings::add_error( __( 'Error: The API certificate is not valid.', 'woocommerce-for-japan' ) );
					return false;
				}
				...
				if ( $valid_until < time() ) {
					WC_Admin_Settings::add_error( __( 'Error: The API certificate has expired.', 'woocommerce-for-japan' ) );
					return false;
				}

				if ( $cert_info['subject']['CN'] != $creds->get_username() ) {
					WC_Admin_Settings::add_error( __( 'Error: The API username does not match the name in the API certificate.  Make sure that you have the correct API certificate.', 'woocommerce-for-japan' ) );
					return false;
				}
				...
				WC_Admin_Settings::add_error( __( 'Error: You must provide API signature or certificate.', 'woocommerce-for-japan' ) );
				return false;
			}
			...
			if ( 'yes' === $settings_array['require_billing'] ) {
				...
				if ( ! $is_account_enabled_for_billing_address ) {
					...
					WC_Admin_Settings::add_error( __( 'The "require billing address" option is not enabled by your account and has been disabled.', 'woocommerce-for-japan' ) );
				}
			}
		}
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( $total_refundable_amount < $amount ) {
			if ( 0 == $total_refundable_amount ) {
				...
				return new WP_Error( 'paypal_refund_error', sprintf( __( 'Refund Error: The requested refund amount is too large. The refund amount must be less than or equal to %s.', 'woocommerce-for-japan' ), html_entity_decode( get_woocommerce_currency_symbol() ) . $total_refundable_amount ) );
			}
		} else {
