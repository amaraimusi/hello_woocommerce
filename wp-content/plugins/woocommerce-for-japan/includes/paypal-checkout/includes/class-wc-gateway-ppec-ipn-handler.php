...
class WC_Gateway_PPEC_IPN_Handler extends WC_Gateway_PPEC_PayPal_Request_Handler {
	...
	public function check_request() {
		try {
			...
			wp_die( $e->getMessage(), esc_html__( 'PayPal IPN Request Failure', 'woocommerce-for-japan' ), array( 'response' => 500 ) );
		}
	}
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) != number_format( $amount, 2, '.', '' ) ) {
			...
			// Put this order on-hold for manual checking.
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'woocommerce-for-japan' ), $amount ) );
			exit;
		}
	}
	...
	protected function payment_status_completed( $order, $posted_data ) {
		...
		if ( 'completed' === $posted_data['payment_status'] ) {
			$this->payment_complete( $order, ( ! empty( $posted_data['txn_id'] ) ? wc_clean( $posted_data['txn_id'] ) : '' ), __( 'IPN payment completed', 'woocommerce-for-japan' ) );
			...
			if ( 'authorization' === $posted_data['pending_reason'] ) {
				$this->payment_on_hold( $order, __( 'Payment authorized. Change payment status to processing or complete to capture funds.', 'woocommerce-for-japan' ) );
			} else {
				$this->payment_on_hold( $order, sprintf( __( 'Payment pending (%s).', 'woocommerce-for-japan' ), $posted_data['pending_reason'] ) );
			}
		}
	}
	...
	protected function payment_status_failed( $order, $posted_data ) {
		$order->update_status( 'failed', sprintf( __( 'Payment %s via IPN.', 'woocommerce-for-japan' ), wc_clean( $posted_data['payment_status'] ) ) );
	}
	...
	protected function payment_status_refunded( $order, $posted_data ) {
		...
		if ( $order->get_total() == ( $posted_data['mc_gross'] * -1 ) ) {
			...
			$order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.', 'woocommerce-for-japan' ), strtolower( $posted_data['payment_status'] ) ) );
			$this->send_ipn_email_notification(
				sprintf( __( 'Payment for order %s refunded', 'woocommerce-for-japan' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order_id . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
				sprintf( __( 'Order #%1$s has been marked as refunded - PayPal reason code: %2$s', 'woocommerce-for-japan' ), $order->get_order_number(), $posted_data['reason_code'] )
			);
		}
	}
	...
	protected function payment_status_reversed( $order, $posted_data ) {
		$order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id();
		$order->update_status( 'on-hold', sprintf( __( 'Payment %s via IPN.', 'woocommerce-for-japan' ), wc_clean( $posted_data['payment_status'] ) ) );
		$this->send_ipn_email_notification(
			sprintf( __( 'Payment for order %s reversed', 'woocommerce-for-japan' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order_id . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
			sprintf( __( 'Order #%1$s has been marked on-hold due to a reversal - PayPal reason code: %2$s', 'woocommerce-for-japan' ), $order->get_order_number(), wc_clean( $posted_data['reason_code'] ) )
		);
	}
	...
	protected function payment_status_canceled_reversal( $order, $posted_data ) {
		$order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id();
		$this->send_ipn_email_notification(
			sprintf( __( 'Reversal cancelled for order #%s', 'woocommerce-for-japan' ), $order->get_order_number() ),
			sprintf( __( 'Order #%1$s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %2$s', 'woocommerce-for-japan' ), $order->get_order_number(), esc_url( admin_url( 'post.php?post=' . $order_id . '&action=edit' ) ) )
		);
	}
