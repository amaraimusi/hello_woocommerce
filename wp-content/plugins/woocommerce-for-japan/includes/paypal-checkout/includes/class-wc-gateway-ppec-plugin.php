...
//		add_action( 'init', array( $this, 'load_plugin_textdomain' ) );
	...
	public function bootstrap() {
		try {
			if ( $this->_bootstrapped ) {
				throw new Exception( __( '%s in WooCommerce Gateway PayPal Checkout plugin can only be called once', 'woocommerce-for-japan' ), self::ALREADY_BOOTSTRAPED );
			}
	...
	protected function _check_dependencies() {
		...
		if ( version_compare( WC()->version, '2.5', '<' ) ) {
			throw new Exception( __( 'WooCommerce Gateway PayPal Checkout requires WooCommerce version 2.5 or greater', 'woocommerce-for-japan' ), self::DEPENDENCIES_UNSATISFIED );
		}
	...
	public function plugin_action_links( $links ) {
		...
		$plugin_links[] = '<a href="https://docs.woocommerce.com/document/paypal-express-checkout/">' . esc_html__( 'Docs', 'woocommerce-for-japan' ) . '</a>';
