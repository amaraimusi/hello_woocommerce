...
class WC_Gateway_PPEC_Admin_Handler {
	...
	public function add_capture_charge_order_action( $actions ) {
		...
		$actions['ppec_capture_charge'] = esc_html__( 'Capture Charge', 'woocommerce-for-japan' );
	...
	public function show_decimal_warning() {
		if ( get_option( 'wc_gateway_ppce_display_decimal_msg', false ) ) {
			...
			<div class="updated fade">
				<p>
					<strong><?php _e( 'NOTE: PayPal does not accept decimal places for the currency in which you are transacting.  The "Number of Decimals" option in WooCommerce has automatically been set to 0 for you.', 'woocommerce-for-japan' ); ?></strong>
				</p>
			</div>
			<?php
		...
		if ( 'ppec_paypal' === $payment_method ) {
			...
			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				...
				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to capture charge!', 'woocommerce-for-japan' ) . ' ' . $result->get_error_message() );
				} else {
					...
					$order->add_order_note( sprintf( __( 'PayPal Checkout charge complete (Charge ID: %s)', 'woocommerce-for-japan' ), $trans_id ) );
				}
			}
		}
	}
	...
	public function cancel_authorization( $order_id ) {
		...
		if ( 'ppec_paypal' === $payment_method ) {
			...
			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				...
				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to void charge!', 'woocommerce-for-japan' ) . ' ' . $result->get_error_message() );
				} else {
					$order->add_order_note( sprintf( __( 'PayPal Checkout charge voided (Charge ID: %s)', 'woocommerce-for-japan' ), $trans_id ) );
				}
			}
		}
	}
	...
	public function display_order_fee_and_payout( $order_id ) {
		...
		<tr>
			<td class="label ppec-fee">
				<?php echo wc_help_tip( __( 'This represents the fee PayPal collects for the transaction.', 'woocommerce-for-japan' ) ); ?>
				<?php esc_html_e( 'PayPal Fee:', 'woocommerce-for-japan' ); ?>
			</td>
			<td width="1%"></td>
			<td class="total">
		...
		<tr>
			<td class="label ppec-payout">
				<?php echo wc_help_tip( __( 'This represents the net total that will be credited to your PayPal account. This may be in a different currency than is set in your PayPal account.', 'woocommerce-for-japan' ) ); ?>
				<?php esc_html_e( 'PayPal Payout:', 'woocommerce-for-japan' ); ?>
			</td>
			<td width="1%"></td>
			<td class="total">
