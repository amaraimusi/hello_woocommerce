...
class WC_Gateway_PPEC_Checkout_Handler {
	...
	public function paypal_billing_details() {
		...
		?>
		<h3><?php _e( 'Billing details', 'woocommerce-for-japan' ); ?></h3>
		<ul>
			<?php if ( ! empty( $checkout_details->payer_details->billing_address ) ) : ?>
				<li><strong><?php _e( 'Address:', 'woocommerce-for-japan' ) ?></strong></br><?php echo WC()->countries->get_formatted_address( $this->get_mapped_billing_address( $checkout_details ) ); ?></li>
			<?php elseif ( ! empty( $checkout_details->payer_details->first_name ) && ! empty( $checkout_details->payer_details->last_name ) ) : ?>
				<li><strong><?php _e( 'Name:', 'woocommerce-for-japan' ) ?></strong> <?php echo esc_html( $checkout_details->payer_details->first_name . ' ' . $checkout_details->payer_details->last_name ); ?></li>
			<?php endif; ?>

			<?php if ( ! empty( $checkout_details->payer_details->email ) ) : ?>
				<li><strong><?php _e( 'Email:', 'woocommerce-for-japan' ) ?></strong> <?php echo esc_html( $checkout_details->payer_details->email ); ?></li>
			<?php else : ?>
			...
			<?php if ( ! empty( $checkout_details->payer_details->phone_number ) ) : ?>
				<li><strong><?php _e( 'Phone:', 'woocommerce-for-japan' ) ?></strong> <?php echo esc_html( $checkout_details->payer_details->phone_number ); ?></li>
	...
	public function paypal_shipping_details() {
		...
		?>
		<h3><?php _e( 'Shipping details', 'woocommerce-for-japan' ); ?></h3>
		<?php
	...
	public function maybe_cancel_checkout_with_paypal() {
		if ( is_cart() && ! empty( $_GET['wc-gateway-ppec-clear-session'] ) ) {
			...
			$notice =  __( 'You have cancelled Checkout with PayPal. Please try to process your order again.', 'woocommerce-for-japan' );
	...
	public function handle_payment_response( $order, $payment ) {
		...
		if ( 'completed' === strtolower( $payment->payment_status ) ) {
			...
			if ( 'authorization' === $payment->pending_reason ) {
				$order->update_status( 'on-hold', __( 'Payment authorized. Change payment status to processing or complete to capture funds.', 'woocommerce-for-japan' ) );
			} else {
				$order->update_status( 'on-hold', sprintf( __( 'Payment pending (%s).', 'woocommerce-for-japan' ), $payment->pending_reason ) );
			}
			if ( $old_wc ) {
